...
A(box([(BODY, "이 다섯 숫자의 공통점: <b>전부 실제 데이터로 계산했고, 코드와 테스트"
              "(208건)로 재현된다.</b> 희망 섞인 추정이 아니라 지금 시장 기준의 실측이다."),
       (SMALL, "①②③ 2026-07-28 기준(융합 12.5년 데이터 · 거래대금 60거래일) · "
...
A(grid(["시나리오", "TER", "매매비용", "현금", "지수 대비 연간 부진"],
       [["기본", Paragraph("45bp", CELLC), Paragraph("30bp", CELLC),
        ...
        [Paragraph("<b>현금 1% 보유</b>", CELL), Paragraph("45bp", CELLC),
         ...
         Paragraph("<b>76bp — 현금 몫이 23bp</b>", CELL)]],
       [30 * mm, 20 * mm, 22 * mm, 16 * mm, 90 * mm]))
A(Spacer(1, 2 * mm))
A(box([(BODY, "<b>현금 1%의 비용은 12.5년 기준 연 23bp</b>로 보수(45bp)의 절반 수준이다. "
       ...
       (BODY, "게다가 <b>하락장에서는 부호가 뒤집혀 방어가 된다.</b> 실측 위기 3구간에서 "
              ...
              "상시 원칙이다."),
       (SMALL, "시뮬 기간 2014-01-02~2026-07-28 (12.5년, 3,084거래일) · 분기 재고정 · "
               "연율화 편도회전율 26% · 편입 종목은 상장 시점에 따라 6→12종목")]))
A(Spacer(1, 5 * mm))
A(Paragraph("용량과 설정단위 — 숫자 두 개", BIG))
A(grid(["항목", "값", "결정 요인"],
       [["소화 가능 AUM", Paragraph("<b>1,766억 원</b>", CELLBC),
         "병목 넥스틴 — 참여율 20%·청산 5일 · <b>정규장 거래대금</b> 기준"],
        ["실측 거래대금 기준", Paragraph("2,329억 원", CELLC),
        ...
        ["설정단위(CU)", Paragraph("<b>20억 원 (20만 좌)</b>", CELLBC),
         "총괴리 9.8bp ≤ 허용 15bp · 0주 종목 없음 · 격자가 굵어 값은 날마다 움직인다"],
        ["정밀도 병목", Paragraph("SK하이닉스", CELLC),
         "주당 155만원 → 1CU에 237주 · 반올림 단위가 굵다(12종목 중 최대)"]],
       [34 * mm, 34 * mm, 110 * mm]))
A(Spacer(1, 3 * mm))
...
A(box([(BODY, "<b>이 문서가 말하지 않은 것 — 한계와 가정.</b>"),
       ...
       (BODY, "· <b>12.5년 수익률은 성과 주장에 쓰지 않는다.</b> 오늘 확정한 구성을 소급 재생한 "
              ...
              "실려 있다. 크기도 쟀다 — 같은 12종목을 동일가중으로 돌리면 +1,117%로, "
              "차이 <b>+2,612%p가 오늘 비중을 소급한 프리미엄</b>이다(counterfactual.csv). "
              "반면 낙폭은 어느 비중이든 -45% 이상으로 같았다 — 이 문서가 수익률이 아니라 "
              "<b>낙폭(-45.2%)·위기 용량(74~187억)·드래그</b>를 앞세우는 이유다."),
       (BODY, "· 실제 ETF 상장은 자산운용사만 가능하다. 개인이 이 구성을 직접 복제하려면 "
              "정수 주 제약 때문에 최소 <b>약 1억 7천만원</b>(비중편차 100bp 허용) 규모가 "
              "필요하다. 이 값은 정수 주 격자라 가격 스냅샷에 민감하다 — 자릿수로만 읽을 것."),
       (SMALL, "본 문서는 학습 목적의 모의 설계서다. 투자 권유가 아니며, 어떤 매매의 근거도 되지 않는다. "
               "모든 수치는 jeongga_hbm_etf 저장소의 코드로 재현 가능하다 (테스트 208건).")],
