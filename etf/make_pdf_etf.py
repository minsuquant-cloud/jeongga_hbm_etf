...
A(box([(BODY, "이 다섯 숫자의 공통점: <b>전부 실제 데이터로 계산했고, 코드와 테스트"
              "(18종 331건)로 재현된다.</b> 희망 섞인 추정이 아니라 지금 시장 기준의 실측이다."),
...
A(box([(BODY, "<b>이 문서가 말하지 않은 것 — 한계와 가정.</b>"),
       ...
       (BODY, "· 위성군 노출도 값은 판정 스냅샷 기준 — 다음 사업보고서에서 갱신된다."),
       (BODY, "· <b>위성군에는 HBM 노출도가 0인 종목(티이엠씨 0.36%)이 있다.</b> 위성군 요건은 "
              "메모리향 비중 70% 이상이지 노출도가 아니기 때문이다 — 이 군의 정의 자체가 "
              "'HBM 귀속 매출 분리가 어려운 전공정 소재·장비'다. 다만 티이엠씨의 경우 "
              "사업보고서가 <b>HBM 수요 강세로 고객사 투자가 DRAM에 쏠리면서 NAND 편중인 "
              "자사 매출은 소폭 감소</b>했다고 적고 있다 — 편입 근거는 '메모리 밸류체인 소속'이지 "
              "'HBM 수혜'가 아니다. 노출도 하한 신설을 검토했으나(3%면 이 종목만 편출) "
              "순도 개선이 0.01%p에 그쳐 규칙을 고칠 값이 아니라고 판단했다. "
              "5% 하한은 주성엔지니어링이 빠지며 용량이 2,249억→968억으로 반토막나 기각."),
       ...
       (SMALL, "본 문서는 학습 목적의 모의 설계서다. 투자 권유가 아니며, 어떤 매매의 근거도 되지 않는다. "
               "모든 수치는 jeongga_hbm_etf 저장소의 코드로 재현 가능하다 (오프라인 테스트 18종 331건).")],
