...
A(Paragraph("지수 1년 실측 경로에 보수·매매비용·현금보유를 얹어 ETF NAV를 시뮬레이션했다. "
...
A(grid(["시나리오", "TER", "매매비용", "현금", "지수 대비 연간 부진"],
       [["기본", Paragraph("45bp", CELLC), Paragraph("30bp", CELLC),
        ...
        [Paragraph("<b>현금 1% 보유</b>", CELL), Paragraph("45bp", CELLC),
         ...
         Paragraph("<b><font color='#B3261E'>174bp — 현금 몫이 113bp</font></b>", CELL)]],
       [30 * mm, 20 * mm, 22 * mm, 16 * mm, 90 * mm]))
A(Spacer(1, 2 * mm))
A(box([(BODY, "연 +280%로 달리는 테마에서는 <b>현금 1%가 보수(45bp)의 2.5배를 갉아먹는다.</b> "
              "따라서 1번 운용 원칙: 현물설정(in-kind) 극대화, 현금 최소화."),
       (SMALL, "추적오차(연율) 기본 시나리오 9.2bp 수준 · 시뮬 기간 2025-07-28~2026-07-27 "
               "(최종 12종목, 지수 +280%) · 현금 드래그는 지수 상승률에 비례하므로 "
               "상승률이 꺾이면 함께 줄어든다")]))
A(Spacer(1, 5 * mm))
A(Paragraph("용량과 설정단위 — 숫자 두 개", BIG))
A(grid(["항목", "값", "결정 요인"],
       [["소화 가능 AUM", Paragraph("<b>1,968억 원</b>", CELLBC),
         "병목 넥스틴(비중 3.3% × 거래대금 65억/일) — 참여율 20%·청산 5일 기준"],
        ["가정을 조이면", Paragraph("591억 원", CELLC),
         "참여율 10%·청산 3일(보수) = 기준의 0.30배 · ADV 반토막까지 겹치면 295억"],
        ["7종목이던 시절", Paragraph("613억 원", CELLC),
         "당시 넥스틴 비중 10.6% — 12종목 재배분으로 용량 3.3배 개선"],
        ["설정단위(CU)", Paragraph("<b>20억 원 (20만 좌)</b>", CELLBC),
         "총괴리 7.1bp ≤ 허용 15bp · 0주 종목 없음 · 관행(5~20만 좌) 상단"],
        ["정밀도 병목", Paragraph("SK하이닉스", CELLC),
         "주당 176만원 → 1CU에 207주 · 반올림 단위가 굵다(-6.9bp, 12종목 중 최대)"]],
       [34 * mm, 34 * mm, 110 * mm]))
A(Spacer(1, 3 * mm))
...
A(box([(BODY, "<b>이 문서가 말하지 않은 것 — 한계와 가정.</b>"),
       (BODY, "· 용량의 거래대금은 최근 60일(강세장) 기준 — 거래가 식으면 용량도 준다. "
              "종가×거래량 근사 사용. 참여율 20%·청산 5일은 관측이 아니라 우리가 고른 "
              "가정이고, 용량은 여기에 정비례한다(조이면 591억)."),
       (BODY, "· 추적오차 시뮬의 리밸런싱은 분기 재고정 근사 — 실제 정기변경(심사)과 다를 수 있다."),
       (BODY, "· 위성 5종목의 노출도 값은 판정 스냅샷 기준 — 다음 사업보고서에서 갱신된다."),
       (BODY, "· 지수 1년 성과(+280%)는 오늘 확정한 구성을 소급 재생한 값이라 종목 선정의 "
