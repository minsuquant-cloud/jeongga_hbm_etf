...
def _fee_cost_factors(bt: pd.DataFrame, ter_bp: float, trade_cost_bp: float,
                      cash_weight: float) -> tuple[pd.Series, pd.Series]:
    ...
    cost_factor = 1.0 - trade_cost_bp / 1e4 * bt["turnover"] * (1.0 - cash_weight)
...
def drag_decomposition(bt: pd.DataFrame, nav: pd.Series,
                       ...
                       cash_weight: float = 0.0) -> pd.Series:
