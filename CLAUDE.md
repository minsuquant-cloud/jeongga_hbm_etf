...
### 결과 — 기존 숫자가 강세장 전용이었음이 드러났다

| 구간 | 지수수익률 | 최대낙폭 | 현금1% 기여 | 종목수 |
|---|---:|---:|---:|---:|
| 전체 12.5년 | +5,150% | -43.5% | +27bp(드래그) | 12 |
| 2018 반도체 사이클 붕괴 | -37.3% | -39.1% | **-73bp(방어)** | 7 |
| 2020 코로나 폭락 | -39.8% | -41.3% | **-328bp(방어)** | 7 |
| 2022 금리인상기 | -34.6% | -40.2% | **-58bp(방어)** | 11 |
...
AUM을 정할 때 강세장 용량(1,994억)이 아니라 위기 용량(74~187억)을 기준으로 봐야 한다.
...
| 항목 | 기존(강세장 1년) | **12.5년 융합** |
|---|---:|---:|
| 추적오차 갭 | 61bp/년 | **69bp/년** (TER 45 + 비용 0.2 + 현금 24) |
| 현금 1% 기여 | +92bp (드래그) | **+24bp** — 강세장 1년이 예외적으로 컸던 것 |
| 용량 (실측 거래대금) | 1,994억 | 2,359억 |
| **용량 (정규장 기준)** | — | **1,789억** ← 설계 기준으로 쓸 것 |
| CU 7억 총괴리 | 6.3bp | **26.8bp** (주가 상승으로 정밀도 악화) |
...
| 허용 편차 | 최소 투자금액 | 실제 편차 | 현금 잔여 |
|---:|---:|---:|---:|
| 100bp | **6,829만원** | 45.8bp | 0.46% |
| 50bp | 1억 4,634만원 | 32.2bp | 0.32% |
| 25bp | 3억 1,219만원 | 10.6bp | 0.11% |

하한을 만드는 것은 **SK하이닉스 179.8만원 × 비중 18.4%** 다. 소액으로는 이 한 종목의
정수 주 제약 때문에 비중이 크게 틀어진다.

### ⚠️ 설계서 PDF는 아직 옛 숫자다

`docs/정가HBM_ETF_설계서.pdf`의 수치는 `make_pdf_etf.py`에 **하드코딩**돼 있고
(CSV를 읽지 않는다), 위 갱신이 반영되지 않았다. 대조가 필요한 항목:
용량 `1,968억` → 정규장 1,789억 · 추적오차 관련 `113bp`/`174bp` · CU 괴리.
