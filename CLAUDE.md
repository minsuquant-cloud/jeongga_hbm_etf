...
**중요 — 이 프로젝트의 성격**: 팀 과제 제출용이 아니라 **사용자가 실제로 만들려는 자기 ETF**다. 모든 숫자는 험한 날 기준까지 검증하는 것을 원칙으로 한다(스트레스 테스트 상시화). 다음 후보: 하락장 실측 계속 추적(조정장 진행 중), S4→정기변경 규칙 문서화, 관찰종목(심텍·SFA) 공시 모니터링.
